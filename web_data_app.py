...
def book_data():
    ...
    titles = []
    prices = []
    # Created a list for the sales price
    sales_price = []
    ...
    for book in book_results:
        ...
        #Implemented sales price according to specifications
        sales_price.append(f"{0.75 * float(book.find('p', class_='price_color').text[1:]):.2f}")

    # Create a DataFrame using the two lists
    book_data = pd.DataFrame(list(zip(titles, prices, sales_price)), columns=['Titles','Prices', 'Sales Price'])    
...
def learn():
    ...
    learnt = "I learnt the principles and framework of data engineering along with the tools available for manipulating data"
    return learnt
    pass
